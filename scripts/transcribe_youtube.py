...
def build_markdown(
    ...
    notes = [
        f"Generated by scripts/transcribe_youtube.py using {result.method}.",
        *result.notes,
    ]
    if fallback_errors:
        notes.append("Fallback attempts before success: " + "; ".join(fallback_errors) + ".")
...
- Processed: {processed}
- Processing notes:
{indent_bullets(notes_text)}

## Metadata

- Video ID: {metadata.get("id", "")}
- Duration: {duration}
- Transcript language: {language_line}
...
This transcript was generated from a public YouTube source. Verify important claims against the original video and official YC materials before treating them as guidance.
"""
